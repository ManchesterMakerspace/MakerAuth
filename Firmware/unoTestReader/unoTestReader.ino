...
#define BUFFER_SIZE 32   // size of all buffers write card, recieve Serial, get UID (96 bytes)
...
void presenceTest(){
  ...
  if(cardReader.PICC_IsNewCardPresent()){
    INTERFACE.print("Last card?:");
    INTERFACE.println(getRequestUID());
    if (cardReader.PICC_ReadCardSerial()){
      INTERFACE.print(count);
      INTERFACE.print(": read card:");
      INTERFACE.println(getRequestUID());
      delay(OPEN_TIME);
      count++;
    }
...
/*void writeCardTest(){
    ...
    if (    piccType != MFRC522::PICC_TYPE_MIFARE_MINI                   // Check for compatibility
        ...
        return;                                          // TODO probably should handle this case
    }
...
void syncGetCardId(){                        // sync get id
    INTERFACE.println(getRequestUID());      // send uid to server
    char* response = syncRecieve();          // wait for a response from server
